...
## 2026-05-02 | 全局文案 | 将“证据链”收口为“记录和凭证”

- 为什么改：
  用户反馈“证据链完整”不像人话，尤其对不熟悉技术和审计语言的个人救助者不友好；一期仍要保持透明记录定位，但前台表达应更像日常记账和补凭证。
- 改了什么：
  将发现卡、客态详情 Hero、首页推荐理由、首页资格原因和当前 IA / PRD / 字段文档中的“证据链 / 证据完整度 / 基础支出证据”统一改成“记录和凭证较齐 / 已有基础凭证 / 还要补一条支出凭证 / 记录凭证情况”。
- 影响范围：
...
## 2026-06-08 | 支出详情 | 修正编辑返回与修改记录编号

- 日期：2026-06-08
- 改动主题：保存修改后回到原支出详情，并修正修改记录排序和次数编号。
- 为什么改：编辑保存时成功反馈和手动返回各退一层，导致用户被带回救助详情；修改记录又用倒序编号，让最新一次显示成第 1 次。
- 改了什么：编辑模式成功反馈不再自动返回，只手动退回支出详情触发远端详情刷新；修改记录按 `editedAt` 从旧到新展示，并按展示顺序显示第 N 次修改；同步补 UI 纯函数测试和支出记录 IA。
- 影响范围：仅影响已发布主态支出编辑后的返回路径、支出详情刷新和修改记录展示；不改云函数、repository、数据模型或草稿支出。
- 验证结果：`format:check`、`lint`、`typecheck`、`test:domain`、targeted UI tests、`build:weapp`、`git diff --check` 通过；全量 `test:ui` 仍只因既有根目录组件直接 import SVG 基线失败。
- 下一步 / 遗留问题：需要在微信开发者工具里实测保存修改后停留在支出详情，并确认从支出详情返回个案详情时金额重新加载。

## 2026-06-09 | 资金表达 | 进度条改为总预算尺

- 日期：2026-06-09
- 改动主题：发现卡、主态资金卡和草稿预览资金条统一改成“总预算 = 100%”。
- 为什么改：用户反馈进度条按预算总长更容易理解；救助人已垫付金额放进条内分段会让人误读当前筹集进度。
- 改了什么：`HomepageCaseCardVM` 移除支持 / 垫付双进度字段，统一消费 `progressPercent = 已确认支持 / 总预算`；发现卡、owner 卡、草稿预览和旧根目录残留组件不再渲染垫付分段；同步更新首页 IA、详情 IA、主 IA、优先级矩阵和总控。
- 影响范围：涉及 selector、VM type、发现页、救助人主页复用卡、主态详情与草稿预览展示；金额文案、资金状态三档、推荐理由、CloudBase、repository 写链和现有页面兼容保持不变，未新增 richer VM / richer mock。
- 验证结果：`typecheck`、`test:domain`、`lint`、`format:check`、`git diff --check` 通过；`test:domain` 覆盖预算尺进度样例。
- 下一步 / 遗留问题：需要真机看首页卡和主态详情首屏，确认用户能区分“预算进度条”和“当前垫付 / 差额”数字。

## 2026-06-09 | 资金表达 | 当前差额改为预算差额

- 日期：2026-06-09
- 改动主题：资金卡当前差额和“即将筹满”状态改按总预算与已确认支持计算。
- 为什么改：预算尺改完后，差额仍用 `已确认支出 - 已确认支持`，导致 `¥4,200` 预算、`¥300` 登记时误显示差额 `¥1,800` 和“即将筹满”。
- 改了什么：`remainingTargetAmount` 收口为 `总预算 - 已确认支持`，资金卡当前差额改消费该字段；资金状态改为 `已达总预算 / 即将筹满 / 距离预算还差较多`，其中“即将筹满”要求确认支持达到预算 80%；工作台、主态卡和草稿预览的第三指标同步改成预算差额。
- 影响范围：涉及 ledger selector、首页卡 VM、客态详情资金卡、owner 资金卡、草稿预览、工作台金额摘要和使用说明文案；`verifiedGapAmount` 保留为垫付差额兼容字段，未新增 richer mock，CloudBase / repository 写链不变。
- 验证结果：`typecheck`、`test:domain`、`lint`、`format:check`、`git diff --check` 通过；domain tests 覆盖低支持不误报“即将筹满”、80% 后才接近筹满、四舍五入到 80% 不误判、支持达到预算才显示已达总预算。
- 下一步 / 遗留问题：需要真机确认截图中的案例应显示当前差额 `¥3,900`，状态为“距离预算还差较多”。

## 2026-06-09 | 资金表达 | 即将筹满改为 300 元尾差

- 日期：2026-06-09
- 改动主题：将“即将筹满”从预算比例判断改为预算差额不超过 300 元。
- 为什么改：80% 或 2000 元这类阈值对不同预算规模都容易误导；用户更容易理解“总预算 - 已确认登记 = 300”才算最后一段。
- 改了什么：资金状态与推荐理由的接近完成条件改为 `0 < remainingTargetAmount <= 300`；首页 / 详情 / 主 IA 同步改成 300 元尾差口径；domain 测试改为覆盖 300 命中、301 不命中。
...
## 2026-06-09 | 资金表达 | 当前缺口文案收口

- 日期：2026-06-09
- 改动主题：端内资金状态和指标文案改成 `当前缺口 / 即将筹满 / 已覆盖预算` 口径。
- 为什么改：`距离预算还差较多`、`当前差额` 和 `当前垫付` 读起来拗口，也容易把预算缺口、已确认支出和救助人垫付混在一起。
- 改了什么：首页卡、客态资金卡、主态资金卡、草稿预览和工作台摘要统一展示 `已确认支出 / 已确认支持 / 当前缺口`；普通缺口直接显示金额，300 元以内显示 `即将筹满`；同步推荐理由、测试和当前 IA / 字段文档。
...
## 2026-06-09 | UX 文案 | 统一档案与记录术语

- 日期：2026-06-09
- 改动主题：把动物级对象统一称为 `档案`，把时间线单条内容保留为 `记录`。
- 为什么改：工作台、详情页、底部导航和按钮中“档案 / 记录”混用，用户难以判断当前是在维护一份动物档案还是一条时间线记录。
- 改了什么：更新底部 tab、新建流程、工作台、主客态详情、档案主页、联系与支持相关提示、fixture/demo 文案，并同步主 IA、页面 IA、项目总览和当前开发文档。
- 影响范围：影响展示文案、分享标题、owner 结束确认文案和当前文档术语；`支出记录 / 记录详情页` 等单条记录概念保持不变。
- 数据层说明：现有页面兼容保持；只调整 repository / adapter / fixtures 中的展示文案和测试断言，未新增 richer VM / richer mock，不改数据结构、selector 或 CloudBase 写链。
- 验证结果：`format`、`typecheck`、`test:domain`、`lint`、`format:check`、`build:weapp`、`git diff --check` 通过；构建仍只有既有图片体积和 no async chunks warning。
- 下一步 / 遗留问题：需要在微信开发者工具里重新编译，重点看底部导航、我的档案、档案详情和档案主页是否都显示新词。
