...
这是一版根据当前 Figma 页面截图和节点结构反推出来的设计 token 草案，不是 Figma 里的正式变量。

当前文件中 `get_variable_defs` 返回为空，说明设计稿里还没有正式 design tokens。所以这份文档不是在还原已有 token，而是给小程序先定一套能落地、后面也能回填到 Figma 的设计语言。
