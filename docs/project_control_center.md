...
- 按最小闭环看：已进入 Alpha 可试跑与回归阶段
- 按 Figma 完整页面覆盖率看：约 `70% - 80%`
...
2. 登记一笔与处理登记在多账号下数据一致
3. 记录票据、更新进展、追加预算提交后跨页回看稳定
4. Alpha Round 0-4 能暴露并消化 P0 / P1 问题
...
| 处理登记页面 | `已可试跑` | `319:1382` | `figma_progress_map` / `pending_field_contracts` | 已完成双 tab 结构版并跑通原生截图；确认 / 未匹配链路已远端验通，继续做卡片精修和多账号回归 |
...
- 近期已完成的前端收口：
  ...
  - 发现卡、客态详情资金卡、主态动物资金卡和草稿预览资金卡的资金进度条已统一成“总预算作为 100% 轨道，已确认支持作为唯一填充”；当前差额按“总预算 - 已确认支持”展示，已确认支出 / 当前垫付只保留为数字指标和垫付压力依据，不再进入条内分段或筹满判断
  - 客态详情页已补 `loading / error` 页面态与真机构建下的运行态排版修正
  - 产品内 SVG 图标已统一收口到语义化 `AppIcon` + lucide 本地资产；发现页 / 详情页状态 badge 和更新进展阶段选择已不再混用 Figma exact SVG 或 emoji 图标
  - 主态详情页已完成 `动物资金卡 + 快捷动作区 + 摘要/详情双态 + 时间线卡 + 底部分享/结束区` 的结构版收口，并已跑通 owner 原生截图
  ...
  - 记账页已补顶部吸顶合计反馈：原合计区滚出可读区域后，导航下方显示轻量迷你合计条，底部 CTA 不再承载合计信息
