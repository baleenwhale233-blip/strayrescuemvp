...
- 昵称：阿宁（小满救助）
- 微信号：`xiaoman_rescue_qa`
- 有小满测试二维码占位图
- 联系备注：加好友请备注“小满 JM520106”
...
| 案例 | 状态 | 默认用途 |
|---|---|---|
| 栗子 | 医疗救助中 | 完整流程：查档、支持登记、核实、记账、预算、只读详情 |
...
| Round 2 | 你 + 1 名协助者 | 25-30 分钟 | 双账号确认、未匹配、手动登记支持、工作台提醒 |
| Round 3 | 你本人 | 20-30 分钟 | 写进展、记账、预算、联系方式与建档发布 |
| Round 4 | 修复后定向复测 | 10-15 分钟 | 失败项 + 1 条完整流程 |
...
- `HT-03 联系路径`
  - 步骤：点击“查看联系方式”
  - 通过标准：用户能找到联系路径，不误以为平台内支付
- `HT-04 登记支持`
  - 步骤：点击“登记支持”，填写金额、称呼、留言、上传测试截图并提交
  - 通过标准：成功提示清楚，回到详情后不怀疑“到底有没有记上”
- 默认案例：`栗子`
- 补充案例：`芝麻` 用来故意测凭证弱和不确定感
...
- 上传权限弹窗是否打断流程
- 键盘遮挡和长文 / 长昵称溢出
- 从微信分享卡片冷启动进入详情页时，左上角返回是否能回到发现页
- “分享档案”默认主按钮、点击“结束”后再右滑确认的交互是否可理解
- 返回后页面刷新是否符合人的预期
- 多账号下多个页面的数据是否同时一致
...
- 结束档案已经改成二段确认保护，但正式结束档案后端 action 还未接入。
