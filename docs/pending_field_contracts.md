...
- 给新线程、接手的 agent 和工程师一个更聚焦的字段待办入口
...
| 字段 | 所属层 | 文字标注 | 当前状态 | 后续动作 |
|---|---|---|---|---|
| `ledger.remainingTargetAmountLabel` | VM | 当前缺口展示值，语义为当前总预算 - 已确认支持 | 已有并已接资金卡 | 当前总预算需包含预算调整后的值；继续保留 `verifiedGapAmountLabel` 作为垫付差额兼容字段，不直接废弃 |
...
| `nickname` | 表单输入 | 支持者称呼，默认从当前用户 profile 昵称 / OPENID 兜底生成，允许覆盖 | 已有 | 当前页面显示时重新读取 profile，优先取“我的”页昵称，其次本机昵称 / OPENID 尾号；不要把联系方式微信号或提示文案作为提交值 |
